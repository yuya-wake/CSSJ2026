...
class TreeBuilderConfig:
    def __init__(
        ...
        # Require summarization model explicitly (CHANGED)
        if summarization_model is None:  # CHANGED: no default summarization model
            raise ValueError(
                "summarization_model must be explicitly provided (no default model)."
            )  # CHANGED
        ...
        if embedding_models is None:
            raise ValueError(
                "embedding_model must be explicitly provided (no default model)."
            )  # CHANGED
        ...
        if len(embedding_models) == 0:  # CHANGED: forbid empty dict
            raise ValueError("embedding_models must not be empty")  # CHANGED
        ...
        # Require cluster embedding model key explicitly (CHANGED)
        if cluster_embedding_model is None:  # CHANGED: no default key like "OpenAI"
            raise ValueError(
                "cluster_embedding_model must be explicitly provided (no default key)."
            )  # CHANGED
        if cluster_embedding_model not in embedding_models:
            raise ValueError(
...
class TreeBuilder:
    ...
    def build_from_text(self, text: str, use_multithreading: bool = True) -> Tree:
        ...
        """
        chunks = split_text(self.tokenizer, text, self.max_tokens)
