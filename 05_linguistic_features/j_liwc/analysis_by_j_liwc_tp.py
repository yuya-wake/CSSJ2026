...
summary_csv_path = os.path.join(MNT_PATH, "linguistic_features", "succeeded_by_summary_tp.csv")
rag_csv_path = os.path.join(MNT_PATH, "linguistic_features", "succeeded_by_rag_tp.csv")
screening_csv_path = os.path.join(MNT_PATH, "linguistic_features", "succeeded_by_screening_tp.csv")
...
for method, csv_path in zip(methods, csv_list):
    ...
    res_no_len.to_csv(os.path.join(results_dir, f"{method}_succeed_wilcoxon_all_features_fdr_bh.csv"), index=False, encoding="utf-8")
    sig_no_len.to_csv(os.path.join(results_dir, f"{method}_succeed_wilcoxon_significant_only_fdr_bh.csv"), index=False, encoding="utf-8")
    ...
    out[keep_cols].to_csv(os.path.join(results_dir, f"{method}_succeed_delta_per_video.csv"), index=False, encoding="utf-8")
