...
class FaissRetrieverConfig:
    def __init__(
        ...
        # Require embedding_model explicitly (CHANGED)
        if embedding_model is None:  # CHANGED
            raise ValueError("embedding_model must be explicitly provided (no default model).")  # CHANGED
        if not isinstance(embedding_model, BaseEmbeddingModel):  # CHANGED
            raise ValueError("embedding_model must be an instance of BaseEmbeddingModel")  # CHANGED

        if question_embedding_model is not None and not isinstance(  # CHANGED
            question_embedding_model, BaseEmbeddingModel  # CHANGED
        ):
            raise ValueError(
        ...
        self.embedding_model = question_embedding_model or embedding_model  # CHANGED
    ...
    def retrieve(self, query: str) -> str:
        ...
        if self.use_top_k:
            ...
            for i in range(range_):
                tokens = len(
                    self.tokenizer(
                        self.context_chunks[indices[0][i]], add_special_tokens=False
                    ).input_ids
                )
                context += self.context_chunks[indices[0][i]]
