...
def get_optimal_clusters(
    ...
    # CHANGED: If there are too few data points to perform clustering, treat it as a single cluster.
    if len(embeddings) < 3:
        return 1
    ...
    for n in n_clusters:
        ...
        # CHANGED: If an error occurs (e.g., variance cannot be computed), skip that number of clusters.
        try:
            gm.fit(embeddings)
    ...
    # CHANGED: If computation fails for all candidate numbers of clusters, fall back to a single cluster.
    if not bics:
        return 1
...
def GMM_cluster(embeddings: np.ndarray, threshold: float, random_state: int = 0):
    ...
    # CHANGED: # If the distribution's variance cannot be computed (e.g., due to a singular matrix), stop and merge into a single cluster
    try:
        gm.fit(embeddings)
        probs = gm.predict_proba(embeddings)
    except ValueError as e:
        logging.warning(f"GMM fitting failed (likely ill-defined covariance): {e}. Fallback to single cluster.")
        # Assign everyone to cluster 0.
...
class RAPTOR_Clustering(ClusteringAlgorithm):
    def perform_clustering(
        ...
        for label in np.unique(np.concatenate(clusters)):
            ...
            total_length = sum(
                [len(tokenizer(node.text, add_special_tokens=False).input_ids) for node in cluster_nodes]
            )
