...
class TreeRetrieverConfig:
    def __init__(
        ...
        num_layers=None,
        start_layer=None,
        query_template=None,  # CHANGED: allow query template for embedding
    ):
        ...
        # Store query template (optional)  # CHANGED
        if query_template is not None and not isinstance(query_template, str):  # CHANGED
            raise ValueError("query_template must be a string or None")  # CHANGED
        self.query_template = query_template  # CHANGED
    ...
    def log_config(self):
        ...
        TreeRetrieverConfig:
            ...
            Query Template: {query_template}  # CHANGED: show query template in logs
        """.format(
            ...
            query_template=self.query_template,  # CHANGED
        )
        return config_log
...
class TreeRetriever(BaseRetriever):
    ...
    def __init__(self, config, tree) -> None:
        ...
        self.config = config  # CHANGED: keep original config object for later use
        self.query_template = getattr(config, "query_template", None)  # CHANGED: optional query template
    ...
    def create_embedding(self, text: str) -> List[float]:
        ...
        """
        # Apply query template before embedding (CHANGED)
        if self.query_template is not None:  # CHANGED: use template only if provided
            text = self.query_template.format(q=text)  # CHANGED: prefix for retrieval embedding
    ...
    def retrieve_information_collapse_tree(self, query: str, top_k: int, max_tokens: int) -> str:
        ...
        for idx in indices[:top_k]:
            ...
            node = node_list[idx]
            node_tokens = len(self.tokenizer(node.text, add_special_tokens=False).input_ids)
