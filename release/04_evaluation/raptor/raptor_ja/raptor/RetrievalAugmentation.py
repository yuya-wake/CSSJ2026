import logging
import pickle
import os  # CHANGED: check file existence
import csv  # CHANGED: for logging generation inputs
...
class RetrievalAugmentationConfig:
    def __init__(
        ...
        tr_context_embedding_model=None,  # CHANGED: no default OpenAI
        tr_embedding_model=None,
        tr_num_layers=None,
        tr_start_layer=None,
        tr_query_template=None,  # CHANGED: query prefix template for embedding
        tr_retrieval_mode="collapsed",  # CHANGED: "collapsed" or "traversal"
        log_csv_path=None,  # CHANGED: path to log generation inputs (optional)
        ...
        tb_cluster_embedding_model=None,  # CHANGED: no default OpenAI
    ):
        ...
        # Validate embedding models (CHANGED)
        if tr_embedding_model is None:
            raise ValueError(
                "tr_embedding_model must be explicitly provided (no default model)."
            )  # CHANGED

        if tr_context_embedding_model is None:
            raise ValueError(
                "tr_context_embedding_model must be explicitly provided (no default model)."
            )  # CHANGED
        ...
        if tree_retriever_config is None:
            tree_retriever_config = TreeRetrieverConfig(
                ...
                query_template=tr_query_template,  # CHANGED: pass query template to TreeRetriever
            )
        ...
        self.tr_query_template = tr_query_template  # CHANGED: store query template
        self.tr_retrieval_mode = tr_retrieval_mode  # CHANGED: store retrieval mode
        self.log_csv_path = log_csv_path  # CHANGED: store csv log path
...
class RetrievalAugmentation:
    ...
    def __init__(self, config=None, tree=None):
        ...
        self.retrieval_mode = getattr(config, "tr_retrieval_mode", "collapsed")  # CHANGED: default retrieval mode
        self.log_csv_path = getattr(config, "log_csv_path", None)  # CHANGED: optional csv logging
    ...
    def retrieve(
        ...
        top_k: int = 10,
        max_tokens: int = 3500,
        collapse_tree: bool = None,  # CHANGED: allow config-driven default
        ...
        # Choose retrieval mode from config if not explicitly specified (CHANGED)
        if collapse_tree is None:  # CHANGED
            collapse_tree = (self.retrieval_mode == "collapsed")  # CHANGED
    ...
    def answer_question(
        ...
        # Log generation inputs to CSV (CHANGED)
        if self.log_csv_path is not None:  # CHANGED
            file_exists = os.path.exists(self.log_csv_path)  # CHANGED
            write_header = (not file_exists) or os.path.getsize(self.log_csv_path) == 0  # CHANGED
            with open(self.log_csv_path, "a", newline="", encoding="utf-8") as f:  # CHANGED
                writer = csv.writer(f)  # CHANGED
                # Write header only once (CHANGED)
                if write_header:
